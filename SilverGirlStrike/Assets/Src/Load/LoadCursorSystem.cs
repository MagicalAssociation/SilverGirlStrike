...
public class LoadCursorSystem : CursorSystem {
    ...
    private void Update()
    {
        ...
        {
            //Easingを使ってy座標の移動をする
            cursorObject.transform.position = new Vector2(this.cursorMove[1].Move(parameter.moveType), this.cursorMove[0].Move(parameter.moveType));
        }
    ...
    void SetMoveCursorEasing()
    {
        ...
        this.cursorMove[0].Set(cursorObject.transform.position.y, base.GetNowParam().transform.position.y - cursorObject.transform.position.y, parameter.moveTime);

        this.cursorMove[1].ResetTime();
        this.cursorMove[1].Set(cursorObject.transform.position.x, base.GetNowParam().transform.position.x - cursorObject.transform.position.x, parameter.moveTime);
    }
    IEnumerator Cancel()
    {
